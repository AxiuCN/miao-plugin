...
|------|------|
| `#胡桃面板 换少女头` | 面板换装 |
| `#胡桃面板 换+30暴击` | 属性数值变换（必须带 `换`/`变`/`改`/`补` 触发；百分比仅对 atk/hp/def 按基础值计算） |
| `#胡桃面板 换+30暴击换-10%大攻击` | 多属性变换（各属性间用 `换` 分隔） |
...
对已获取面板数据的角色圣遗物按评分排序展示，支持按角色重算评分、全群收集、套装过滤（`列表` / `统计` 可互换，`遗器` 对应星铁）。

| 指令 | 说明 |
|------|------|
| `#圣遗物列表` | 本人全部角色圣遗物，按原权重评分排序（fork 前为原版行为） |
| `#胡桃圣遗物列表` | 本人全部圣遗物，**按胡桃评分权重重算**（支持角色别名） |
| `#胡桃圣遗物全局列表` | 全群成员圣遗物（需群内使用），按胡桃权重重算 |
| `#圣遗物全局列表` | 全群成员圣遗物，按各自原权重排序 |
| 末尾追加套装名 | 过滤指定套装，如 `#胡桃圣遗物列表魔女4` |

- `#<角色>圣遗物统计` / `#雷神遗器列表` 等同义指令同样支持
- 重算评分依赖角色评分规则（`artis-mark` 权重），便于跨角色比较同一件圣遗物的价值
- 展示数量由 `#喵喵设置圣遗物数量` 控制（默认 28）
